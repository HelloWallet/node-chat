...
extend(Server.prototype, {
	listen: function(port, host) {
		...
		this.httpServer.get('/channels', function(request, response) {
			var titles = [];
			for (var i in _this.channels) {
				titles.push({title: _this.channels[i].title, path: _this.channels[i].basePath});
			}
			response.simpleJSON(200, { titles: titles });
		});
	},
